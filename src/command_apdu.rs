...
pub fn new_command_apdu<'a>(class: &'a Class, instruction: &'a dyn Instruction, p1: u8, p2: u8, le: Option<u8>, command_data: Option<&'a[u8]>) -> CommandAPDU<'a> {
    CommandAPDU{
        class,
        instruction,
        p1,
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>, CommandAPDUError> {
        ...
        if self.command_data.is_some() {
            ...
            if command_data_bytes_len > 255 {
                return Err(CommandAPDUError::IllegalCommandDataLength(command_data_bytes_len));
            }
...
#[cfg(test)]
mod test {
    use crate::class::{ClassTypeForStandardLogicalChannels, new_standard_class, SecureMessagingIndicationForStandardLogicalChannels};
    ...
    fn should_construct_command_with_data() {
        ...
        let apdu = new_command_apdu(&class, &instruction, 0x00, 0x04, Option::None, Option::Some(&command_data));
        let result = apdu.to_bytes();
        assert_eq!(result.unwrap(), Vec::from([0x00, 0xa4, 0x00, 0x04, 0x02, 0x6f, 0x61]));
    }
}
